...
/**
 * A fixed-height screen. Height rather than aspect-ratio on purpose: an aspect
 * box sized itself from the column width and then clipped whatever content did
 * not fit, which cropped the third row at some widths. A fixed height lets the
 * three previews line up with each other and guarantees the content inside
 * them is the thing that decides the size.
 */
function Frame({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      role="presentation"
      className={cn(
        "flex h-[11.5rem] flex-col rounded-xl border border-border-gray bg-bg-surface p-3.5",
        className,
      )}
    >
      {children}
    ...
    <Frame className={className}>
      ...
      <div className="flex min-h-0 flex-1 flex-col justify-center gap-1.5">
        {rows.map((row) => (
          <div
            key={row.course}
            className="flex items-center justify-between gap-2 rounded-md border border-border-gray bg-bg-deep px-2.5 py-1.5"
          >
      ...
      <div className="mt-2.5 flex shrink-0 items-center gap-1.5 border-t border-border-gray pt-2.5">
        ...
        <span className="text-[10px] leading-none text-text-tertiary">
          Faculty-governed · course-scoped
        </span>
      </div>
    </Frame>
  );
...
export function ProfessorPreview({ className }: { className?: string }) {
  ...
  return (
    <Frame className={className}>
      ...
      <div className="flex min-h-0 flex-1 flex-col justify-center gap-3">
        {topics.map((item) => (
          <div key={item.topic} className="flex flex-col gap-1.5">
            <span className="truncate text-[10.5px] font-medium leading-none text-text-primary">
              {item.topic}
            </span>
      ...
      <div className="mt-2.5 flex shrink-0 items-center gap-1.5 border-t border-border-gray pt-2.5 text-[10px] leading-none text-text-tertiary">
        <ArrowUpRight className="h-3 w-3 shrink-0 text-accent" aria-hidden="true" />
  ...
  return (
    <Frame className={className}>
      ...
      <div className="flex min-h-0 flex-1 flex-col justify-center gap-2">
        <p className="ml-auto max-w-[88%] rounded-lg rounded-br-sm bg-accent/10 px-2.5 py-1.5 text-[10.5px] leading-4 text-text-primary">
          Why divide by n−1 and not n?
        </p>
        <div className="rounded-lg rounded-bl-sm border border-border-gray bg-bg-deep px-2.5 py-2">
          <p className="text-[10.5px] leading-4 text-text-secondary">
            The sample mean is estimated from the same data, so one degree of freedom is already
            spent.
          </p>
          <span className="mt-2 inline-flex items-center gap-1 rounded border border-accent/25 bg-accent/[0.07] px-1.5 py-0.5 text-[9px] font-medium leading-none text-accent">
